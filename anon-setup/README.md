...
`anon-setup-enhanced` es un script de bash profesional diseñado para maximizar el anonimato y reducir la huella digital en máquinas virtuales durante pruebas de penetración, ejercicios de Red Team y auditorías de seguridad. Implementa múltiples capas de ofuscación y hardening siguiendo las mejores prácticas de la industria.
...
# Hacer ejecutable
chmod +x anon-setup-enhanced.sh

# Ejecutar (requiere root)
sudo ./anon-setup-enhanced.sh
```
...
sudo cp anon-setup-enhanced.sh /usr/local/bin/anon-setup
...
sudo anon-setup-enhanced.sh --verbose
sudo anon-setup-enhanced.sh -v

# Modo silencioso (para scripts/cron)
sudo anon-setup-enhanced.sh --quiet
sudo anon-setup-enhanced.sh -q

# Restaurar configuración original
sudo anon-setup-enhanced.sh --restore
sudo anon-setup-enhanced.sh -r

# Saltar verificación de VPN
sudo anon-setup-enhanced.sh --skip-vpn-check

# Ver ayuda
sudo anon-setup-enhanced.sh --help
sudo anon-setup-enhanced.sh -h
```
...
```
anon-setup/
├── anon-setup-enhanced.sh     # Script principal v2.0
├── anon-setup.sh              # Script legacy v1.0
...
4. **Abre un nuevo Issue** con:
   - Versión del script (`grep VERSION anon-setup-enhanced.sh`)
...
- 📖 [Wiki del Proyecto](https://github.com/TraceVoid/Anon-Setup/wiki)
- 💬 [Discusiones en GitHub](https://github.com/TraceVoid/Anon-Setup/discussions)
- 🐛 [Reportar Bug](https://github.com/TraceVoid/Anon-Setup/issues/new?template=bug_report.md)
- ✨ [Solicitar Feature](https://github.com/TraceVoid/Anon-Setup/issues/new?template=feature_request.md)
...
Por favor, envía un reporte privado a: [security@yourproject.com](mailto:security@yourproject.com)
...
- **macchanger** - Por la herramienta de cambio de MAC
- **iptables** - Por el framework de firewall
- **Comunidad de infosec** - Por feedback y sugerencias
...
- 💬 [Discord](https://discord.gg/yourserver) - Chat en tiempo real
- 🐦 [Twitter](https://twitter.com/yourhandle) - Actualizaciones
- 📺 [YouTube](https://youtube.com/yourchannel) - Tutoriales
